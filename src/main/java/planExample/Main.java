	...
	static void print4GOpenCv(AStarPlanner a){
		try{
			...
			for(int control = 0; control < ty.size(); ++control){
				...
				Random n = new Random();
				double rx = (n.nextInt(254 - 0 + 1) + 0);
				double ry = (n.nextInt(254 - 0 + 1) + 0);
				double rz = (n.nextInt(254 - 0 + 1) + 0);
				
				Scalar randomScalar = new Scalar(rx, ry, rz, 0);
				...
				for(int p = te.size()-1; p >= 0 ; --p){
					...
					if(p==0){
						opencv_imgproc.circle(mat, new Point(pos[0]+14, pos[1]+14), step/6, randomScalar, -1, opencv_core.LINE_4, 0);
					}if(temp.value == 'G' || temp.value == 'S'){
						opencv_imgproc.circle(mat, new Point(pos[0]+14, pos[1]+14), step/6, randomScalar, -1, opencv_core.LINE_4, 0);
					}else{
						opencv_imgproc.circle(mat, new Point(pos[0], pos[1]), step/3, randomScalar, -1, opencv_core.LINE_4, 0);
					}
			...
			String pah = Main.class.getProtectionDomain().getCodeSource().getLocation().getPath();
